(() => {
    ...
    function initShowcaseOrbit() {
        ...
        function updateRotation(deltaX, timestamp) {
            const deltaTime = Math.max(
                16,
                timestamp - (state.lastTime || timestamp),
            );
    ...
    function initThreeBackground() {
        ...
        try {
            ...
            const spheres = [];
            const palette = [
                0xf8d8de, 0xf5c8cf, 0xd4c3ea, 0xf8efad, 0xd8e6d2, 0xefe7db,
            ];
            for (let i = 0; i < 42; i++) {
            ...
            window.addEventListener(
                ...
                (e) => {
                    pointerX =
                        (e.clientX / Math.max(window.innerWidth, 1) - 0.5) *
                        0.8;
                    pointerY =
                        (e.clientY / Math.max(window.innerHeight, 1) - 0.5) *
                        0.6;
                },
                {
                    passive: true,
            ...
            function tick() {
                ...
                spheres.forEach((s, i) => {
                    s.position.y +=
                        Math.sin(t + s.userData.angle + i) *
                        s.userData.speed *
                        0.6;
                    s.position.x +=
                        Math.cos(t * 0.8 + s.userData.angle) *
                        s.userData.drift *
                        0.4;
