(() => {
	...
	const initFaq = () => {
		try {
			...
			lines.forEach((line) => {
				if (!(line instanceof HTMLElement) || line.dataset.faqBound === 'true') {
					return;
				}
				...
				if (!(button instanceof HTMLButtonElement) || !(answer instanceof HTMLElement)) {
					console.warn('[FAQ] 必要要素が不足している行をスキップしました。', line);
					return;
				}
				...
				button.addEventListener('click', () => {
					try {
						...
						console.error('[FAQ] 開閉処理でエラーが発生しました。', error);
					}
				});
			});
			...
			window.addEventListener(
				'resize',
				() => {
					document.querySelectorAll('.line.is-open .answer').forEach((answer) => {
