window.addEventListener("load", () => {
	...
	const validData = imageData.filter((item, index) => {
		...
		if (!isValid) {
			console.warn(`imageData[${index}] は無効なためスキップします:`, item);
		}
	...
	parent.style.display = parentStyle.display === "inline" ? "inline-block" : parentStyle.display;
	...
	async function changeImage() {
		...
		try {
			...
			overlayImg.style.objectFit = window.getComputedStyle(targetImg).objectFit || "cover";
